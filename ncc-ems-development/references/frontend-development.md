...
新建的自定义参照前端目录（如 `src/ems/refer/<module>/<refName>/index.js`）只放文件不会生效：页面不会请求对应 `GridRefAction.do`。必须同时：

1. 在 `hbwork/config.json` 的 `patch.path`（或等价构建入口）中加入该 `index.js`；
...
3. 重新启动前端构建（仅热更新或 `Ctrl+F5` 不够，新增入口必须重启 `npm run dev`/重新打包）。
...
- 合同号来源：`QC_REPORTBILL_B.VBDEF7`（案例中写入质量异议 `def1`，映射以实际页面字段为准）。
- 选择质检报告后调用详情 Action，返回值必须与 `setFormItemsValue` 字段名一致。
- 更换或清空质检报告时同步清空旧合同号，避免残留。

## 联系人按最新主职去重

内部联系人参照若出现一人多岗，从 `BD_PSNJOB` 按人员取 `INDUTYDATE` 最新且 `ISMAINJOB='Y'` 的记录，每人只显示一行，再按岗位名称过滤（如 `POSTNAME LIKE '%工段长%'` 或 `%车间主任%`）。
