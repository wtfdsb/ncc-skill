...
1. 服务启动时间是否早于 class 部署时间；是则旧 class 仍在内存。
2. 用 `javap -p -c <class>` 确认部署产物包含预期方法/字符串，不只看文件时间。
3. 源文件编码：GBK 工程不要用 UTF-8 重写；编译参数 `-encoding` 必须与源文件一致，否则中文注释/字符串乱码或编译异常。
4. 前端新增参照入口后必须重启 dev server 或重新打包，不能只 `Ctrl+F5`。
