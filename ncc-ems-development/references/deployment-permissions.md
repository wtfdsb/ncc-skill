...
常见案例：`ISupplierdeductionMaintain`、`IQualityobjectionService`、`ISSCIntelligentApvService` 未找到时，对应 lczj/ems/审批服务模块未部署或启动未完成。

## 前端补丁包结构

NCC 前端补丁 ZIP 必须同时包含两套前端资源目录，只放一套会导致页面 404：

```text
replacement/hotwebs/resources/ems/<module>/...
replacement/hotwebs/nccloud/resources/ems/<module>/...
```
...
新增自定义参照时，除上述资源外还要把参照 `index.js` 加入前端 `config.json` 的 `patch.path`，并用项目自带 `config/patch.js` 重新打包，不能只复制 `src`。
