...
| `LCZJ_SUPPLIERDEDUCTION_H` | 扣款单表体，`VBDEF1-20`、`CSOURCEBILLID/BID`、`REASON/MEMO`；`VBDEF16` 常存来源质量异议主键 |
| `QC_REPORTBILL_B` | 质检报告表体，案例中合同号在 `VBDEF7` |
| `V_MATERIAL_PRICE_BJDJ` | 物料单价视图，`PK_MATERIAL`、`NQTORIGTAXPRICE`（含税单价） |
| `PO_ARRIVEORDER` | 到货主表，`PK_ARRIVEORDER`、`VBILLCODE`、`PK_PUPSNDOC`（采购员） |
...
| `PUB_WORKFLOWNOTE` | 审批待办表，`BILLID`、`CHECKMAN`、`DEALDATE`、`ISCHECK`、`PK_WF_TASK` |
| `PUB_WF_TASK` | 工作流任务，`PK_WF_TASK`、`ACTIVITYDEFID` |
| `BD_DEFDOC` / `BD_DEFDOCLIST` | 自定义档案（例 `EMS_SCRAPFLOW`），`BD_DEFDOC.CODE/NAME` 关联 `BD_DEFDOCLIST.CODE` |
| `SM_PUB_FILESYSTEM` | 附件元数据，`PK_DOC`、`FILEPATH`、`ISFOLDER`、`ISDOC`、`DR` |
| `EMS_SPAREPARTSCRAP_DETAILS` | 备件报废明细主表，`PK_SCRAP`、`SCRAPCODE` |
...
- 质量异议 `PK_ARRIVALBILL` 即到货单主键，规则里可直接使用。
